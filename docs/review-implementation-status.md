...
The corrective implementation is in the working tree over
`6422cb7beeca6cb124bb5be6f53f2d675b3d549e`, including the ongoing UX changes that
were present when the review began. Existing owner plans and scratch files were
preserved. This is developer work, not a published or physically qualified
release candidate. The local qualification results below cover the complete
working-tree implementation at `0.9.0.dev0`. The owner subsequently authorized
the GitHub update and installation on the BC250. That follow-through uses
`0.9.0.dev1`; its hosted CI and deployment results will be recorded here after
verification. Physical release qualification remains pending.

| Phase | Implemented changes | Remaining qualification |
| --- | --- | --- |
| RV-0 | Complete pytest inventory, omission detection, machine-readable skips/results, default + slow CI, current-state index | Hosted CI execution on the eventual immutable candidate |
...
| RV-4 | Admission before handler creation; bounded HTTP framing/body/response/SSE; exact in-flight reservations across rate windows; enforced generation cap; interruptible response deadlines | Independent security review and long-running hostile-traffic qualification |
...
| RV-10 | Local regression, real Tk, compile and clean-package checks | Candidate freeze plus C4/C5/C6/C8 remain pending |
